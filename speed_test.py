...
def calculate_speed(self, trk_id, track,box):
    ...
    if self.trk_previous_times[trk_id] != 0 and direction != "unknown" and trk_id not in self.trk_idslist:
        ...
        if time_difference > 0:
            ...
            # Extract the area inside the bounding box
            box_region = self.im0[y1:y2, x1:x2]

            # Save the box region
            cv2.imwrite(f"violators/box_{trk_id}.jpg", box_region) 
...
videofile = "temp/vecteezy_car-and-truck-traffic-on-the-highway-in-europe-poland_7957364.mp4"
...
# Video writer
video_writer = cv2.VideoWriter(f"{videofile}-speed_estimation.mp4",
